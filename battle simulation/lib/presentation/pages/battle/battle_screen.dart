...
class _BattleScreenState
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isSetup
                ? viewModel.config.isBattle
                      ? '${viewModel.config.summaryLabel}. Red attacks from the top and Blue defends from the bottom.'
                      : '${viewModel.config.summaryLabel}. Red and Blue both compress toward the center and win by elimination.'
  ...
  Widget _buildCell({
    ...
    final VoidCallback? onTap = isSetup
        ? (editable
              ? () => _showPlacementSheet(
                  context: context,
                  row: row,
              ...
              : null)
        : (snapshotUnit != null && definition != null
              ? () => _showReplayUnitDialog(
        ...
        child: Container(
          ...
          child: LayoutBuilder(
            builder: (BuildContext context, BoxConstraints constraints) {
              ...
              if (!isSetup && snapshotUnit != null && side != null) {
                return _buildReplayTile(
                  definition: definition,
                  unit: snapshotUnit,
                  side: side,
                  tinyTile: tinyTile,
  ...
  Widget _buildReplayTile({
    ...
    required BattleUnitStateEntity unit,
    required ArmySide side,
    required bool tinyTile,
  }) {
    final double healthRatio = unit.maxHealth == 0
        ? 0
        : (unit.currentHealth / unit.maxHealth).clamp(0.0, 1.0);
    final List<_BattleTileBadgeData> badges = _buildTemporaryBadgeData(unit);
    ...
    return ClipRRect(
      ...
      child: Stack(
        children: [
          Positioned.fill(
            child: Container(
              color: side == ArmySide.armyA
                  ? const Color(0xFFD75168)
                  : const Color(0xFF3B62D0),
            ),
          ),
          Positioned.fill(
            child: Align(
                  ...
                  ...badges
                      ...
                      .map(
                        (badge) => Padding(
                          ...
                          child: _buildBadge(badge, tinyTile),
                        ),
                      ),
                  if (badges.length > 2)
                    _buildOverflowBadge(badges.length - 2, tinyTile),
                ],
              ),
            ),
        ],
  ...
  List<_BattleTileBadgeData> _buildTemporaryBadgeData(
    BattleUnitStateEntity unit,
  ) {
    final List<_BattleTileBadgeData> badges = <_BattleTileBadgeData>[];
    if (unit.fireStacks > 0) {
      badges.add(
        const _BattleTileBadgeData(
          label: 'Fire',
          icon: Icons.local_fire_department_rounded,
          color: Color(0xFFE97832),
        ),
      );
    }
  ...
  Widget _buildBadge(_BattleTileBadgeData badge, bool tinyTile) {
    final double size = tinyTile ? 14 : 16;
    return Container(
      width: size,
      height: size,
      decoration: BoxDecoration(
      ...
      child: Icon(badge.icon, size: tinyTile ? 9 : 10, color: Colors.white),
    );
  }

  Widget _buildOverflowBadge(int extraCount, bool tinyTile) {
    final double size = tinyTile ? 14 : 16;
    return Container(
      width: size + 6,
      height: size,
  ...
  Future<void> _showReplayUnitDialog({
    required BattleUnitDefinitionEntity definition,
    required BattleUnitStateEntity unit,
  }) {
        ...
        return Dialog(
          ...
          child: ConstrainedBox(
            ...
            child: BattleUnitCard(
              ...
              healthValue: unit.currentHealth,
              maxHealthValue: unit.maxHealth,
              highlightTaunt: unit.hasTaunt,
              onKeywordTap: _showKeywordTooltip,
            ),
          ),
        );
      },
  ...
  void _showKeywordTooltip(String glossaryKey) {
    final BattleKeywordGlossaryEntry? entry = BattleKeywordGlossary.lookup(
      glossaryKey,
    );
    if (entry == null) {
      return;
